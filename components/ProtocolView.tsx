  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
      <div className="border-b border-slate-200 pb-5 flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        {selectedIds.size > 0 && (
          <button onClick={handleBatchProtocol} disabled={isBatchProcessing} className="px-8 py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest flex items-center gap-2 shadow-xl shadow-indigo-200 transition-all active:scale-95">
      ...
      <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex flex-col md:flex-row items-center gap-4">
        <div className="relative flex-1 w-full">
          ...
          <input type="text" placeholder="Pesquisar por prefixo, fornecedor ou secretaria..." className="w-full pl-12 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500 font-bold text-xs uppercase" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        </div>
        <div className="flex items-center gap-4 w-full md:w-auto">
          <button onClick={toggleSelectAll} className="px-4 py-3 bg-slate-100 hover:bg-slate-200 rounded-xl text-[10px] font-black text-slate-600 uppercase tracking-widest flex items-center gap-2 whitespace-nowrap transition-all">
      ...
      {pendingRecords.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {pendingRecords.map((record) => (
            <div key={record.id} onClick={() => toggleSelect(record.id)} className={`bg-white rounded-[2.5rem] border-2 shadow-sm transition-all group overflow-hidden relative cursor-pointer flex flex-col ${selectedIds.has(record.id) ? 'border-indigo-500 bg-indigo-50/10' : 'border-slate-200 hover:border-indigo-200'}`}>
               ...
               <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-6">
                  ...
                  <DetailItem icon={<Hash size={14} />} label="Nº Orç. Aprovado" value={record.num_orc_aprovado} highlight />
                  <DetailItem icon={<Tag size={14} />} label="Nota Fiscal" value={record.nota_fiscal || 'NÃO INFORMADO'} />
                  <DetailItem icon={<DollarSign size={14} />} label="Valor Total" value={`R$ ${record.valor_total}`} bold />
               ...
               <div className="p-6 pt-0 mt-auto">
                  {inconsistencyId === record.id ? (
                    <div className="bg-slate-900 p-4 rounded-2xl space-y-3 animate-in slide-in-from-top-2 duration-300">
                       ...
                       <textarea autoFocus value={inconsistencyNote} onChange={e => setInconsistencyNote(e.target.value)} onClick={e => e.stopPropagation()} className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-white text-xs font-bold uppercase outline-none focus:ring-1 focus:ring-indigo-500" placeholder="EX: NF DIVERGENTE DO VALOR..." rows={2} />
                       <button onClick={(e) => { e.stopPropagation(); handleSaveInconsistency(record); }} disabled={isSavingInconsistency} className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-2">
                    ...
                    <div className="flex gap-3">
                       <button onClick={(e) => { e.stopPropagation(); setInconsistencyId(record.id); setInconsistencyNote(''); }} className="flex-1 py-4 bg-slate-100 hover:bg-amber-100 text-slate-500 hover:text-amber-700 rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] flex items-center justify-center gap-2 transition-all active:scale-95">
                          <AlertTriangle size={18} />
                          Inconsistência
                       </button>
                       <button onClick={(e) => { e.stopPropagation(); handleProtocolAction(record.id); }} disabled={processingId === record.id} className="flex-[2] py-4 bg-slate-900 hover:bg-black text-white rounded-2xl font-black uppercase text-[10px] tracking-[0.2em] flex items-center justify-center gap-2 transition-all active:scale-95 shadow-lg group-hover:bg-indigo-600">
                          {processingId === record.id ? <Loader2 className="animate-spin" size={18} /> : <ClipboardCheck size={18} />}
